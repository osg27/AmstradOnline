...
export default function LobbyPage() {
  ...
  const visibleShelves = useMemo(() => PLATFORM_SHELVES.map((platform) => ({
    ...
    eras: platform.eras.map((era) => ({
      ...
      systems: era.systems.filter((system) => (
        (!system.adminOnly || isAdmin) && (!system.superAdminOnly || isSuperAdmin || (system.id === 'cpc_pinball' && (isAdmin || isXyphoe)))
      )).map((system) => {
  ...
  async function createSession(mode = selectedMode) {
    ...
    try {
      ...
      if (!selectedSystem || selectedSystem.locked || !modeConfig?.enabled || (selectedSystem.adminOnly && !isAdmin) || (selectedSystem.superAdminOnly && !isSuperAdmin && !(selectedSystem.id === 'cpc_pinball' && (isAdmin || isXyphoe))) || (modeConfig.testing && !canUsePreviewSystems)) {
