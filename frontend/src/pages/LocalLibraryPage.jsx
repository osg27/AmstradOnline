...
export default function LocalLibraryPage({ embedded = false, onboarding = false, onComplete = null }) {
    ...
    async function loadLibrary() {
      try {
        ...
        const savedGames = mismatchedAlienIds.size
          ...
          : loadedGames;
        // One-time migration from thousands of VIP rows to a single catalogue snapshot.
        // Do not hold the first render up while IndexedDB compacts the legacy cache.
        void migrateLegacyVipLibraryGames(savedGames).catch(() => {});
        ...
        const savedVipSnesGames = savedGames.filter((game) => game.source === 'vip-snes-gameplay');
        ...
        if (isVip) {
          if (hasCompleteVipCache) {
            ...
            setGames([...localGamesWithArtwork, ...savedVipMameGames, ...savedVipC64Games, ...savedVipAmstradGames, ...savedVipSpectrumGames, ...savedVipMastersystemGames, ...savedVipNesGames, ...savedVipSnesGames, ...savedVipMegadriveGames, ...savedVipPcengineGames, ...currentVipAmigaGames]);
          ...
          const mastersystemSortedFiles = (Array.isArray(mastersystemCatalog.games) ? mastersystemCatalog.games : [])
            .filter((entry) => entry?.file_name)
            .map((entry) => {
              const fileName = entry.file_name;
              return {
                id: `vip-mastersystem-file:${fileName}`,
                name: fileName,
                path: fileName,
                extension: '7z',
                size: Number(entry.bytes) || 0,
          ...
          const mastersystemGames = groupReleaseFiles(mastersystemSortedFiles)
            .flatMap((sortedGame) => sortedGame.releases.map((release) => {
              const primary = release.media[0];
              return {
                id: `vip-mastersystem:${release.id}:${primary.name}`,
                title: sortedGame.title,
                fileName: primary.name,
                path: `VIP Master System No-Intro/${primary.name}`,
                size: Number(primary.size) || 0,
                system: 'mastersystem',
                roomSystem: 'mastersystem',
                source: 'vip-mastersystem-nointro',
                sorterScore: release.score,
                sorterWarnings: release.warnings,
              };
            }))
          ...
          const snesSortedFiles = (Array.isArray(snesCatalog.games) ? snesCatalog.games : [])
            .filter((entry) => entry?.file_name)
            .map((entry) => {
              const fileName = entry.file_name;
              return {
          ...
          const snesGames = groupReleaseFiles(snesSortedFiles)
            .flatMap((sortedGame) => sortedGame.releases.map((release) => {
              const primary = release.media[0];
              return {
                id: `vip-snes:${release.id}:${primary.name}`,
                title: sortedGame.title,
                fileName: primary.name,
                path: `VIP SNES Gameplay/${primary.name}`,
                size: Number(primary.size) || 0,
                system: 'snes',
                roomSystem: 'snes',
                source: 'vip-snes-gameplay',
                sorterScore: release.score,
                sorterWarnings: release.warnings,
              };
            }))
          ...
          const pcengineSortedFiles = (Array.isArray(pcengineCatalog.games) ? pcengineCatalog.games : [])
            .filter((entry) => entry?.file_name)
            .map((entry) => {
              const fileName = entry.file_name;
              return {
                id: `vip-pcengine-file:${fileName}`,
                name: fileName,
                path: fileName,
                extension: '7z',
                size: Number(entry.bytes) || 0,
          ...
          const pcengineGames = groupReleaseFiles(pcengineSortedFiles)
            .flatMap((sortedGame) => sortedGame.releases.map((release) => {
              const primary = release.media[0];
              return {
                id: `vip-pcengine:${release.id}:${primary.name}`,
                title: sortedGame.title,
                fileName: primary.name,
                path: `VIP PC Engine No-Intro/${primary.name}`,
                size: Number(primary.size) || 0,
                system: 'pcengine',
                roomSystem: 'pcengine',
                source: 'vip-pcengine-nointro',
                sorterScore: release.score,
                sorterWarnings: release.warnings,
              };
            }))
  ...
  async function scanSystemFolder(targetSystem, directoryHandle, existingFolder = null) {
    let matchingFolder = existingFolder;
    if (!matchingFolder && typeof directoryHandle.isSameEntry === 'function') {
      const storedFolders = await getLocalLibraryFolders();
      for (const folder of storedFolders.filter((item) => item.system === targetSystem.id && item.handle)) {
        try {
          if (await directoryHandle.isSameEntry(folder.handle)) {
            matchingFolder = folder;
            break;
          }
        } catch {
          // A stale handle should not prevent another folder being connected.
        }
      }
    }
    await scanSystemEntries(
      targetSystem,
      directoryHandle.name,
      walkDirectory(directoryHandle),
      directoryHandle,
      matchingFolder?.id,
    );
  }

  async function scanSystemEntries(targetSystem, folderName, entries, directoryHandle, existingFolderId = null) {
    try {
      const uniqueFolderId = globalThis.crypto?.randomUUID?.()
        || `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
      const folderId = existingFolderId || `folder:${targetSystem.id}:${uniqueFolderId}`;
      const nextGames = [];
      const sampleHandles = [];
      let scanned = 0;
  ...
  async function rescanLibraryFolder(folder) {
    const system = SYSTEM_BY_ID[folder.system];
    if (!system || !folder.handle) {
      setStatus('This browser needs you to choose that folder again before it can be rescanned.');
      return;
    }
    try {
      let permission = typeof folder.handle.queryPermission === 'function'
        ? await folder.handle.queryPermission({ mode: 'read' })
        : 'granted';
      if (permission !== 'granted' && typeof folder.handle.requestPermission === 'function') {
      ...
      await scanSystemFolder(system, folder.handle, folder);
    } catch (err) {
      setStatus(`Could not rescan ${folder.name}: ${err.message}`);
    }
  }

  async function clearLibraryFolder(folderId) {
    ...
    const removedGameIds = new Set(
      games
        .filter((game) => game.folderId === folderId)
        .map((game) => game.id),
    );
    const nextFolders = folders.filter((item) => item.id !== folderId);
    const nextGames = games.filter((game) => game.folderId !== folderId);
    ...
    try {
      ...
      setStatus(`${folder.name} removed from ${system?.label || folder.systemLabel || 'your library'}. Your ROM files were not changed.`);
    } catch (err) {
      setStatus(`Could not remove ${folder.name}: ${err.message}`);
    }
  }
  ...
  async function launchGame(game) {
    ...
    try {
      ...
      if (game.source === 'vip-pcengine-nointro') {
        setVipPreparation({
          badge: 'VIP PC Engine',
          title: game.title,
          fileName: game.fileName,
          label: 'Downloading sorted PC Engine release',
          loaded: 0,
          total: game.size || 0,
          percent: 0,
          attempt: 1,
        });
        await prepareVipPcengineFile(game.fileName, ({ loaded, total, attempt, retrying }) => {
          const expectedTotal = total || game.size || 0;
          setVipPreparation({
            badge: 'VIP PC Engine',
            title: game.title,
            fileName: game.fileName,
            label: retrying ? 'Retrying PC Engine download...' : 'Downloading sorted PC Engine release',
            loaded,
            total: expectedTotal,
            percent: expectedTotal ? Math.min(100, Math.round((loaded / expectedTotal) * 100)) : 0,
            attempt,
        ...
        setVipPreparation({
          ...
          title: game.title,
          fileName: game.fileName,
          label: 'PC Engine release ready — opening room',
          loaded: game.size || 1,
          total: game.size || 1,
          percent: 100,
          attempt: 1,
      ...
      if (game.source === 'vip-mastersystem-nointro') {
        setVipPreparation({
          badge: 'VIP Master System', title: game.title, fileName: game.fileName,
          label: 'Downloading sorted Master System release', loaded: 0,
          total: game.size || 0, percent: 0, attempt: 1,
        });
        await prepareVipMastersystemFile(game.fileName, ({ loaded, total, attempt, retrying }) => {
          const expectedTotal = total || game.size || 0;
          setVipPreparation({
            badge: 'VIP Master System', title: game.title, fileName: game.fileName,
            label: retrying ? 'Retrying Master System download...' : 'Downloading sorted Master System release',
        ...
        setVipPreparation({
          badge: 'VIP Master System', title: game.title, fileName: game.fileName,
          label: 'Master System release ready — opening room', loaded: game.size || 1,
                      ...
                      {folder.system ? (
                        <div className="library-folder-actions">
                          {folder.handle ? (
                            <button
                              type="button"
                              className="secondary folder-action-button"
                              onClick={() => rescanLibraryFolder(folder)}
                              title={`Rescan ${folder.name}`}
                          ...
                          <button
                            ...
                            onClick={() => clearLibraryFolder(folder.id)}
