...
function buildAmigaKickstartPayload(system, fileName, bytes, supportKickstarts = []) {
...
async function loadA500WHDLoadKickstart() {
  const stored = await loadStoredKickstart(AMIGA_KICKSTART_KEY);
...
export default function RoomPage() {
  ...
  const reloadAmigaAgaFrame = useCallback(async () => {
    ...
    if (storedKickstart) {
      const supportKickstarts = kickstartStorageKey === AMIGA_AGA_KICKSTART_KEY
        ? await loadA500WHDLoadKickstart()
        : [];
      frame.contentWindow?.postMessage(
        buildAmigaKickstartPayload(roomSystem, storedKickstart.fileName, storedKickstart.bytes, supportKickstarts),
      ...
      try {
        ...
        const payload = isX68000
          ...
          : isAtariSt
            ...
            : buildAmigaKickstartPayload(roomSystem, storedKickstart.fileName, storedKickstart.bytes, amigaSupportKickstarts);
  ...
  async function saveAmigaScoreNow() {
    try {
      setAmigaScoreBusy(true);
      setAmigaScoreStatus(`Closing the run and saving ${amigaScoreGame?.title || 'Amiga'} scores...`);
      const flushResult = await emulatorFrameRef.current?.contentWindow?.flushAmigaWHDLoadSave?.();
      if (!flushResult?.flushed) {
        addLog(`${amigaScoreGame?.title || 'Amiga'} WHDLoad save flush was unavailable; checking existing score files.`);
      } else {
        addLog(`${amigaScoreGame?.title || 'Amiga'} WHDLoad quit sent; checking the committed WHDSaves score table.`);
      }
      setAmigaScoreBusy(false);
  ...
  async function handleDiskSelected(event) {
    try {
      ...
      const allowedExtensions = isPuaeAmiga
        ...
        : isMasterSystem ? ['.sms', '.zip', '.7z'] : isMegaDrive ? ['.bin', '.gen', '.md', '.smd', '.zip', '.7z'] : isNes ? ['.nes', '.zip', '.7z'] : isSnes ? ['.sfc', '.smc', '.fig', '.swc', '.bsx', '.gd3', '.gd7', '.dx2', '.zip', '.7z'] : isPcEngine ? ['.pce', '.sgx', '.zip', '.7z'] : isX68000 ? ['.dim', '.img', '.d88', '.88d', '.hdm', '.dup', '.2hd', '.xdf', '.hdf', '.cmd', '.m3u', '.zip'] : isPlayStation ? ['.cue', '.bin', '.chd', '.pbp', '.iso', '.zip', '.7z'] : isSaturn ? ['.cue', '.bin', '.chd', '.iso', '.zip', '.7z'] : isC64 ? ['.d64', '.g64', '.f64', '.t64', '.p00', '.p01', '.tap', '.prg', '.crt', '.zip', '.7z'] : isMsx ? ['.rom', '.mx1', '.mx2', '.dsk', '.cas', '.m3u', '.zip', '.7z'] : isAtari8 ? ['.atr', '.xfd', '.atx', '.xex', '.com', '.car', '.rom', '.bin', '.cas', '.zip', '.7z'] : isAtariSt ? ['.st', '.msa', '.stx', '.ipf', '.zip', '.7z'] : isArcade ? ['.zip', '.7z'] : isSpectrum ? ['.tap', '.tzx', '.z80', '.sna', '.szx', '.zip', '.7z'] : ['.dsk', '.zip'];
  ...
  async function handleKickstartSelected(event) {
    try {
      ...
      const supportKickstarts = amigaRequiredModel === 'A1200'
        ? await loadA500WHDLoadKickstart()
        : [];
      forwardInputToEmulator(buildAmigaKickstartPayload(roomSystem, file.name, bytes, supportKickstarts));
      ...
      <div className={`page-social-layout room-social-layout ${supportsMameScoreboard && isSoloMode ? 'solo-arcade-layout' : ''}`}>
        ...
        {(loadedDiskName || isAmigaFamily || isDiscConsole || isAtariSt) ? (
          <div className="session-strip">
            ...
            {isMsx ? <span>Powered by <a href="https://webmsx.org" target="_blank" rel="noreferrer">WebMSX</a></span> : null}
