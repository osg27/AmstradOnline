...
function findControlProfileMatches(fileName, profiles) {
  if (!fileName || !profiles.length) return [];
...
export default function RoomPage() {
  ...
  const [amstradControlProfiles, setAmstradControlProfiles] = useState([]);
  const [controlProfileStatus, setControlProfileStatus] = useState('idle');
  ...
  useEffect(() => {
    ...
    fetch('/amstrad-controls/index.json?v=2026-06-16-1')
      ...
      .then((profiles) => {
        if (cancelled) return;
        setAmstradControlProfiles(Array.isArray(profiles) ? profiles : []);
        setControlProfileStatus('ready');
      })
      .catch((err) => {
        if (cancelled) return;
        setControlProfileStatus('error');
        addLog(`Amstrad control profiles unavailable: ${err.message}`);
      });
  ...
  function chooseControlProfile(profile) {
    ...
    setControlProfileDrawerOpen(true);
    addLog(`Selected Amstrad controls: ${profile.title}`);
  }
    ...
    <div className="page room-page">
      <div className="page-social-layout room-social-layout">
        ...
        {(loadedDiskName || isAmigaFamily || isPlayStation) ? (
          <div className="session-strip">
            ...
            {isCpcSystem && !selectedControlProfile && controlProfileMatches.length > 0 ? (
              <button type="button" className="secondary control-profile-pill" onClick={() => setControlProfileDrawerOpen(true)}>
                Choose controls ({controlProfileMatches.length})
              </button>
            ) : null}
        ...
        {controlProfileDrawerOpen && isCpcSystem ? (
          <aside className="control-profile-drawer" aria-label="Amstrad controls">
            <div className="control-profile-header">
              <div>
                <span>Amstrad controls</span>
            ...
            {selectedControlProfile ? (
              <>
                <div className="control-profile-meta">
                  <span>{selectedControlProfile.supportsJoystick ? 'Joystick' : 'Keyboard'}</span>
                  <span>{selectedControlProfile.redefinableKeys ? 'Redefinable' : 'Fixed keys'}</span>
