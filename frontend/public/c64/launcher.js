(function () {
  ...
  let statusText = 'Commodore 64 ready';
  ...
  function buildPad(index, mask) {
    ...
    return {
      id: `Old Style C64 Joystick ${index + 1}`,
      index,
      connected: true,
      mapping: 'standard',
  ...
  function configureEmulator(fileName, romUrl) {
    ...
    window.EJS_paths = {
      ...
      'vice_x64sc-wasm.data': '/c64/vice_x64sc-wasm.data',
      'vice_x64sc.json': '/c64/vice_x64sc.json',
    };
    ...
    window.EJS_ready = () => {
      console.log('Old Style Gaming C64: VICE WASM ready');
    };
    ...
    window.EJS_onExit = () => {
      drawStatus('Commodore 64 stopped', fileName);
    };
  }
  ...
  async function loadCurrentRom() {
    ...
    drawStatus('Checking VICE runtime', currentRom.fileName);
    try {
      await preflightEmulatorJs();
    } catch (error) {
      drawStatus('VICE runtime missing', error.message);
      return;
    }
    ...
    drawStatus('Loading Commodore 64', currentRom.fileName);

    loaderScript = document.createElement('script');
    loaderScript.src = `/emulatorjs/data/loader.js?v=${Date.now()}`;
    loaderScript.async = true;
    loaderScript.onerror = () => drawStatus('Commodore 64 failed to load', 'Could not load the VICE WASM host');
  ...
  window.addEventListener('error', (event) => {
    ...
    drawStatus('Commodore 64 error', message);
  });
