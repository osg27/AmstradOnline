(function () {
  const screen = document.getElementById('snes-screen');
  ...
  if (OriginalAudioContext) {
    ...
    if (originalConnect && !window.__oldStyleSnesAudioPatched) {
      window.__oldStyleSnesAudioPatched = true;
  ...
  window.getSnesAudioStream = function getSnesAudioStream() {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (message.type === 'snes_start') {
      window.getSnesAudioStream();
      return;
    }
