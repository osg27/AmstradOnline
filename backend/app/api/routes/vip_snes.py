...
router = APIRouter(prefix="/auth/vip/snes", tags=["vip-snes"])
ARCHIVE_ITEM = "gameplay-snes"
ARCHIVE_DOWNLOAD_ROOT = f"https://archive.org/download/{ARCHIVE_ITEM}"
CATALOG_PATH = Path(__file__).resolve().parents[2] / "data" / "snes_vip_catalog.json"
...
def load_catalog() -> dict:
    try:
        ...
        raise HTTPException(status_code=503, detail="SNES VIP catalogue is unavailable") from exc
    return {"source": ARCHIVE_ITEM, "games": games}


@router.get("/catalog")
def get_catalog(response: Response, _user: User = Depends(require_vip)):
    # The archive listing is deployed with the application and is immutable between releases.
...
def get_archive_file(filename: str, _user: User = Depends(require_vip)):
    if not SAFE_GAME_NAME.fullmatch(filename):
        raise HTTPException(status_code=404, detail="SNES game not found")
    ...
    try:
        ...
        if exc.code == 404:
            raise HTTPException(status_code=404, detail="SNES game not found") from exc
        raise HTTPException(status_code=502, detail="Internet Archive SNES download failed") from exc
    except (URLError, TimeoutError) as exc:
        raise HTTPException(status_code=502, detail=f"Internet Archive SNES download failed: {exc}") from exc
    headers = {
