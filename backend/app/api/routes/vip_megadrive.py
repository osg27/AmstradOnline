...
router = APIRouter(prefix="/auth/vip/megadrive", tags=["vip-megadrive"])

ARCHIVE_ITEM = "SegaGenesisMegaDriveRomCollectionByGhostware"
ARCHIVE_DOWNLOAD_ROOT = f"https://archive.org/download/{ARCHIVE_ITEM}"
CATALOG_PATH = Path(__file__).resolve().parents[2] / "data" / "megadrive_vip_catalog.json"
...
def load_catalog() -> dict:
    try:
        ...
        raise HTTPException(status_code=503, detail="Mega Drive VIP catalogue is unavailable") from exc
...
def get_archive_file(filename: str, _user: User = Depends(require_vip)):
    if not SAFE_GAME_NAME.fullmatch(filename):
        raise HTTPException(status_code=404, detail="Mega Drive game not found")
    ...
    try:
        ...
        if exc.code == 404:
            raise HTTPException(status_code=404, detail="Mega Drive game not found") from exc
        raise HTTPException(status_code=502, detail="Internet Archive Mega Drive download failed") from exc
    except (URLError, TimeoutError) as exc:
        raise HTTPException(status_code=502, detail=f"Internet Archive Mega Drive download failed: {exc}") from exc
