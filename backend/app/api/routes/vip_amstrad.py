...
router = APIRouter(prefix="/auth/vip/amstrad", tags=["vip-amstrad"])

ARCHIVE_ITEM = "AmstradCPCGameCollectionByGhostware"
ARCHIVE_DOWNLOAD_ROOT = f"https://archive.org/download/{ARCHIVE_ITEM}"
CATALOG_PATH = Path(__file__).resolve().parents[2] / "data" / "amstrad_vip_catalog.json"
...
def load_catalog() -> dict:
    try:
        ...
        raise HTTPException(status_code=503, detail="Amstrad CPC VIP catalogue is unavailable") from exc
...
def get_archive_file(filename: str, _user: User = Depends(require_vip)):
    if not SAFE_GAME_NAME.fullmatch(filename):
        raise HTTPException(status_code=404, detail="Amstrad CPC game not found")
    ...
    try:
        ...
        if exc.code == 404:
            raise HTTPException(status_code=404, detail="Amstrad CPC game not found") from exc
        raise HTTPException(status_code=502, detail="Internet Archive Amstrad download failed") from exc
    except (URLError, TimeoutError) as exc:
        raise HTTPException(status_code=502, detail=f"Internet Archive Amstrad download failed: {exc}") from exc
